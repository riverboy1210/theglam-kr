...
async function captureWebflowSite() {
  ...
  try {
    console.log('Starting advanced Webflow site capture...');
    ...
    const isCaptchaPresent = await page.evaluate(() => {
      return document.body.innerText.includes('Confirm you\'re not a bot') || 
             document.body.innerText.includes('Press & Hold');
    });
    
    if (isCaptchaPresent) {
      console.log('WARNING: Page is protected by PerimeterX bot detection');
      report.captureNotes.push('Site is protected by PerimeterX bot detection - full content capture blocked');
      
      // Try to extract what we can from the page
      const pageData = await capturePage(page, BASE_URL, 'home');
    ...
    const reportPath = path.join(EVIDENCE_DIR, 'site-capture-report-advanced.json');
...
async function capturePage(page, url, pageName) {
  ...
  const screenshotPath = path.join(EVIDENCE_DIR, `${pageName}-full-advanced.png`);
  ...
  const pageData = await page.evaluate(() => {
    ...
    while (node = walker.nextNode()) {
      ...
      if (text && text.length > 0 && !text.includes('window._px')) {
    ...
    document.querySelectorAll('img').forEach(img => {
      if (img.src && !img.src.includes('data:')) images.push(img.src);
    });
  ...
  // Save page HTML
  const htmlPath = path.join(EVIDENCE_DIR, `${pageName}-source-advanced.html`);
  const html = await page.content();
  fs.writeFileSync(htmlPath, html);
  console.log(`HTML saved: ${htmlPath}`);
  
  // Save text content
  const textPath = path.join(EVIDENCE_DIR, `${pageName}-text-advanced.txt`);
